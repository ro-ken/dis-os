...
class COCOEvaluator:
    ...
    def evaluate(
        ...
        #tensor_type = torch.cuda.HalfTensor if half else torch.cuda.FloatTensor            #zjx
        tensor_type = 'cpu'
        model = model.eval()
        if half:
            model = model.half()
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                #imgs = imgs.type(tensor_type)                  #zjx
                ...
                outputs = postprocess(
                    outputs, self.num_classes, self.confthre, self.nmsthre, class_agnostic=True             # 用nms吧  #zjx
                )
                if is_time_record:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        '''inference_time = statistics[0].item()            #zjx
