...
def main(exp, args):
    ...
    #量化
    qconfig = {"iteration": 1, "firstconv": False, "use_avg": False, "data_scale": 1.0, "mean": [0, 0, 0],
               "std": [1, 1, 1], "per_channel": False}
    model = mlu_quantize.quantize_dynamic_mlu(model, qconfig, dtype="int8", gen_quant=True)
    #model = torch_mlu.core.mlu_quantize.quantize_dynamic_mlu(model, qconfig, dtype="int8", gen_quant=True)     #全称
