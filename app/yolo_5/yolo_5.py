...
Usage - sources:
    $ python path/to/detect.py --weights yolov5l.pt --source 0              # webcam
                                                             img.jpg        # image
                                                             vid.mp4        # video
                                                             path/          # directory
                                                             path/*.jpg     # glob
...
model = DetectMultiBackend(ROOT / 'yolov5s.pt', device=device, dnn=False, data=ROOT / 'data/coco128.yaml')
...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if pt or jit:
        ...
        model.model.half() if False else model.model.float()
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            save_path = str(save_dir / file)  # im.jpg
            count += 1
