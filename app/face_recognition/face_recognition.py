...
class Face_Recognizer:
    ...
    def draw_name(self, img_rd,target_names):
        ...
        for i in range(self.current_frame_face_cnt):
            ...
            if self.current_frame_face_name_list_bak[i] in target_names:
                draw.text(xy=self.current_frame_face_name_position_list[i], text='目标人物：' + self.current_frame_face_name_list[i],
    ...
    # 显示中文名字 / Show names in chinese
    def show_chinese_name(self):
        name_len = len(self.current_frame_face_name_list)
        for i in range(name_len):
            _name = self.current_frame_face_name_list[i]
            if _name in name_dict.keys():
                name = name_dict[_name].encode('utf-8').decode()
                self.current_frame_face_name_list[i] = name  # 写入中文名
    ...
    def face_recognition(self, img_rd, target_names, frame_cnt):
        ...
        self.current_frame_face_name_list_bak = self.current_frame_face_name_list.copy()
